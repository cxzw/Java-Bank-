...
/*
 * 1.在练习 5_续 1 修改，原来的 Customer 类包含一个称作 account 的关联属性，可用该属性控制一个 Account 对象。
 * 	    重写这个类，使其包含两个关联属性：savingAccount 和 checkingAccount，这两个属性的缺省值是 null
 *2.包含两个访问方法：getSaving 和 getChecking，这两个方法分别返回储蓄和支票总数。
